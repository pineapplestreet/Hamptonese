...
# our results
results <- readRDS("round5 - Entropy/results2.Rds") 
...
# Hampton v Words
ggplot(results %>% filter(n <=32) %>% 
         mutate(entropy=log(entropy)) %>%
         filter(item %in% c("Hamptonese (Method A)","Hamptonese (Method B)","Book of Psalms (words)","Book of Nonsense (words)")), aes(x=n, y=entropy, color=item)) +
  geom_point() +
  geom_line() +
  #scale_y_continuous(trans='log') +
  #scale_x_continuous(trans='log') + 
